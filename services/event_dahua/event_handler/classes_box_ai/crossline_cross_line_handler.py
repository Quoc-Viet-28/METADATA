...
from app.services.event_dahua.image_helper import crop_image, remap_coordinates
from app.utils.image_processing import convert_BytesIO_to_webp
from app.utils.minio_utils import MinioServices
from app.models.event_model import Event


@EventHandlerBoxAIFactory.register_class(["CrossLineDetection"])
class CrossLineHandler:
    ...
    async def handle_data_vehicle(self, minio_service, data, image, name_file_iso, flow_1, flow_2):
        ...
        vehicle_data_save = {
            ...
            'Object_Detect': {
                'Vehicle': {
                    f'Vehicle_{flow_1}': data_vehicle_cross_line,
            ...
            'Image_Vehicle_CrossLine': {
                f'Image_Vehicle_CrossLine_{flow_1}': url_vehicle,
                f'Image_Vehicle_CrossLine_{flow_2}': None
            },
            'Image_Human_CrossLine': None
        }
        return vehicle_data_save
    ...
    async def handle_data_human(self, minio_service, data, image, name_file_iso, flow_1, flow_2):
        ...
        human_data_save = {
            ...
            'Object_Detect':
                {
                    'Vehicle': None,
                    'Human': {
                        f'Human_{flow_1}': data_human_cross_line,
            ...
            'Image_Vehicle_CrossLine': None,
            'Image_Human_CrossLine': {
                f'Image_Human_CrossLine_{flow_1}': url_human,
                f'Image_Human_CrossLine_{flow_2}': None
            },
        }
        return human_data_save
    async def combine_events(self, event1, event2):
        ...
        for key in all_keys:
            ...
            if key == 'Direction_Flow':
            ...
            elif key =="Image_Human_CrossLine":
                if event1.data.get('Image_Human_CrossLine', None) and event2.data.get('Image_Human_CrossLine', None) is not None:
                    combined_data["Image_Human_CrossLine"] = {
                        f"Image_Human_CrossLine_{event1.data.get('Direction_Flow')}": value1.get(f"Image_Human_CrossLine_{event1.data.get('Direction_Flow')}",  None),
                        f"Image_Human_CrossLine_{event2.data.get('Direction_Flow')}": value2.get(f"Image_Human_CrossLine_{event2.data.get('Direction_Flow')}", None)
                    }
                else:
                    combined_data["Image_Human_CrossLine"] = None
            elif key =="Image_Vehicle_CrossLine":
                if event1.data.get('Image_Vehicle_CrossLine', None) and event2.data.get('Image_Vehicle_CrossLine', None) is not None:
                    combined_data['Image_Vehicle_CrossLine'] = {
                        f"Image_Vehicle_CrossLine_{event1.data.get('Direction_Flow')}": value1.get(f"Image_Vehicle_CrossLine_{event1.data.get('Direction_Flow')}",None),
                        f"Image_Vehicle_CrossLine_{event2.data.get('Direction_Flow')}": value2.get(f"Image_Vehicle_CrossLine_{event2.data.get('Direction_Flow')}", None)
                    }
                else:
                    combined_data['Image_Vehicle_CrossLine'] = None
            elif key == "Object_Detect":
                if event1.data.get("Name_Event") == "CrossRegionDetection" and event2.data.get(
                        "Name_Event") == 'CrossRegionDetection':
                    if event1.data.get('Object_Detect', {}).get('Human', None) is not None and event2.data.get('Object_Detect', {}).get('Human', None) is not None:
                    ...
                    elif event1.data.get('Object_Detect', {}).get('Vehicle', None) is not None and event2.data.get('Object_Detect', {}).get('Vehicle', None) is not None:
                        combined_data["Object_Detect"] = {
                            'Human': None,
                            'Vehicle': {
                ...
                elif event1.data.get("Name_Event") == "CrossLineDetection" and event2.data.get(
                        "Name_Event") == "CrossLineDetection":
                    if event1.data.get('Object_Detect', {}).get('Human', None) is not None and event2.data.get('Object_Detect', {}).get('Human', None) is not None:
                        combined_data["Object_Detect"] = {
                            'Human': {
                                f"Human_{event1.data.get('Direction_Flow')}": value1.get('Human').get(
    ...
    async def save(self, data, image, device):
        ...
        if data.get('DetectLine', None) is not None:
            coors = data.get('DetectLine', None)
            url_line = await self.draw_region_on_image(coors, url, name_big_image)
        flow_list = ['LeftToRight', 'RightToLeft']
        ...
        new_event = Event(
            event_type=EventTypeEnum.CROSS_LINE,
            data=data_save,
        ...
        if object_ID_current and name_event_current:
            ...
            if matching_event is None:
                ...
                print(f"Đã thêm {combined_event} vào MongoDB.")
